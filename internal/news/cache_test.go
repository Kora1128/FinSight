...
func TestCache_Cleanup(t *testing.T) {
	...
	recommendations := []struct {
	...
	}{
		{
			...
			rec: Recommendation{
				...
				CreatedAt: now.Add(-200 * time.Millisecond), // Expired
			},
		},
		{
			key: "key-2",
	...
	for _, item := range recommendations {
		cache.Set(item.key, item.rec)
	}

	// Wait for cleanup
	time.Sleep(150 * time.Millisecond)

	// Verify expired recommendation is removed
	_, found := cache.Get("key-1")
	if found {
		t.Error("Expected expired recommendation to be removed")
	}

	// Verify non-expired recommendation is still present
	_, found = cache.Get("key-2")
	if !found {
		t.Error("Expected non-expired recommendation to be present")
	}
}
